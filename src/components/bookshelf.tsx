...
const Bookshelf = ({ books, handles }: BookShelfProps) => {
  ...
  return (
    <main className='p-12 text-slate-800'>
      ...
      <div className='flex flex-wrap gap-12 justify-center items-center mb-2'>
        <Chart value={numBooksRead} nameValue='Readings' total={books.length} nameTotal='Remaining' label='Readings' colors={['#006AB3', '#cad5e2']} />
        <Chart value={readingsY} nameValue='In this year' total={numBooksRead} nameTotal='Readings in others years' label={'In ' + yearNow} colors={['#E63431', '#cad5e2']} />
        <Chart value={readingsM} nameValue='In this month' total={readingsY} nameTotal='Readings in others months of this years' label={'In ' + new Date().toLocaleString('en-US', { month: 'long' })} colors={['#FBAC0F', '#cad5e2']} />
      </div>
      <p className="text-center font-bold mb-12 text-xl">
        <span className="bg-gradient-to-r from-[#006AB3] via-[#E63431] to-[#FBAC0F] bg-clip-text text-transparent">R$ {totalSpent}</span>
      </p>
