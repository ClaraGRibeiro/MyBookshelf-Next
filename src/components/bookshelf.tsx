...
const Bookshelf = ({ books, handles }: BookShelfProps) => {
    ...
    const sortedBooks = [ // cria um array com os livros reorganizados juntando os elementos espalhados
        ...books.filter((b) => b.status === 'Reading'), // espalha livros e pega apenas os 'Reading'
        ...[...books] // cria um array cópia dos livros (espalhando-os), faz os procedimentos e espalha seus elementos
            .filter((b) => b.status !== 'Reading') // sem ser 'Reading'
            .sort((a, b) => {
                if (!sortBy) return 0
    ...
    return (
        <main className='p-12 text-slate-800'>
            ...
            <Table>
                ...
                <TableHeader>
                    <TableRow>
                        <TableHead onClick={() => handleSort('title')} className='text-slate-600 cursor-pointer hover:text-slate-800 group'>
                            <span className='flex items-center flex-row gap-1'>
                                Book Title
                                <CaretSortIcon className='inline group-hover:scale-130 duration-200' />
                            </span>
                        </TableHead>
                        <TableHead onClick={() => handleSort('author')} className='text-slate-600 cursor-pointer hover:text-slate-800 group'>
                            <span className='flex items-center flex-row gap-1'>
                                Author Name
                                <CaretSortIcon className='inline group-hover:scale-130 duration-200' />
                            </span>
                        </TableHead>
                        <TableHead onClick={() => handleSort('publisher')} className='text-slate-600 cursor-pointer hover:text-slate-800 group'>
                            <span className='flex items-center flex-row gap-1'>
                                Publisher Name
                                <CaretSortIcon className='inline group-hover:scale-130 duration-200' />
                            </span>
                        </TableHead>
                        <TableHead onClick={() => handleSort('pages')} className='text-slate-600 cursor-pointer text-center hover:text-slate-800 group'>
                            <span className='flex items-center justify-center flex-row gap-1'>
                                Pages
                                <CaretSortIcon className='inline group-hover:scale-130 duration-200' />
                            </span>
                        </TableHead>
                        <TableHead onClick={() => handleSort('price')} className='text-slate-600 cursor-pointer text-center hover:text-slate-800 group'>
                            <span className='flex items-center justify-center flex-row gap-1'>
                                Price
                                <CaretSortIcon className='inline group-hover:scale-130 duration-200' />
                            </span>
                        </TableHead>
                        <TableHead onClick={() => handleSort('status')} className='text-slate-600 cursor-pointer text-center hover:text-slate-800 group'>
                            <span className='flex items-center justify-center flex-row gap-1'>
                                Status
                                <CaretSortIcon className='inline group-hover:scale-130 duration-200' />
                            </span>
                ...
                <TableBody>
                    {sortedBooks.map((b) => (
                        <TableRow key={b.id} className={b.status === "Reading" ? 'bg-slate-200 hover:bg-slate-300' : ''}>
                            ...
                            <TableCell className='whitespace-nowrap overflow-hidden truncate max-w-32'>{b.publisher ?? '-'}</TableCell>
