    ...
    <Drawer>
      <DrawerTrigger>
        <Button asChild className='' variant='ghost'>
          <Tooltip>
            <TooltipTrigger asChild className='opacity-50 hover:opacity-100 active:opacity-100 z-2 cursor-pointer duration-200 p-0 hover:bg-transparent active:bg-transparent'>
              <span>
      ...
      <DrawerContent>
        <div className='mx-auto h-[90%] w-full overflow-y-auto pb-12'>
          <DrawerHeader>
            ...
            <DrawerDescription>Track reading progress: month by month, year by year.</DrawerDescription>
          </DrawerHeader>
          <div className='mt-12 md:mt-16 flex flex-col gap-12 items-center justify-center'>
            ...
            <Charts type='bar' dataChart={booksByYear} label='Books per Year' colors={['#E63431']} />
            <Charts type='bar' dataChart={booksByMonth} label={'Books per Month (' + new Date().getFullYear() + ')'} colors={['#FBAC0F']} />
          </div>
        </div>
      </DrawerContent>
    </Drawer >
